"""
Script to compare cysteine data.
...
desktop_data = np.genfromtxt(r"Outputs\20260910\Cysteine_Ada\3.csv", skip_header=1, delimiter=",")
desktop_wavelength = desktop_data[:, 1]
desktop_CD = desktop_data[:, 9]
desktop_g = desktop_data[:, 10]
desktop_weak_mask = np.genfromtxt(
        r"Outputs\20260910\Cysteine_Ada\3.csv",
        skip_header=1, delimiter=",",
        usecols=12, dtype=str
    ) == "True"
desktop_abs = desktop_data[:, 11]

CD_data = np.genfromtxt(r"Outputs\20260910\Cysteine_Ada\CD.txt", skip_header=1)
